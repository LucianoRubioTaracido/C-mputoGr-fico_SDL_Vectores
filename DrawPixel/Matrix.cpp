...
void Matrix::ModMatrix(float value, int ro, int co)
{
	mat[ro][co] = value;
}
...
Matrix Matrix::operator* (Matrix& mtx)
{
	if (cols == mtx.rows)
	{
		...
		{
			for (unsigned short j = 0; j < mtx.cols; j++)
			{
				int total = 0;
				for (unsigned short p = 0; p < cols; p++)
				{
					temp.ModMatrix(total + (mat[i][p] * mtx.operator()(p, j)), i, j);
					total = mat[i][p] * mtx.operator()(p, j);
				}
			}
		}
		return temp;
...
//Vector Matrix::operator* (Vector& vect)
//{
//	Vector tempVect = Vector(0, 0);
//	for (unsigned short i = 0; i < rows; i++)
//	{
//		int total = 0;
